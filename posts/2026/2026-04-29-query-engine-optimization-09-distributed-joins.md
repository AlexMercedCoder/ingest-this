...
![Shuffle join redistributing both tables by join key hash so matching keys land on the same node](/images/2026/query-engine-optimization/09-distributed-joins/shuffle-join.png)
...
![Broadcast join copying the small dimension table to every compute node while the large fact table stays in place](/images/2026/query-engine-optimization/09-distributed-joins/broadcast-join.png)
...
![Hash join build and probe phases showing hash table construction from the smaller table and probing from the larger table](/images/2026/query-engine-optimization/09-distributed-joins/hash-join-mechanics.png)
