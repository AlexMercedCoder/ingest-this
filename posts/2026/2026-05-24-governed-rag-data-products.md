...
![Governed RAG architecture showing user/AI agent flowing through query rewrite and intent classification, splitting into retrieval from vector store via data product catalog and policy check, and structured data query via policy check and semantic layer, both assembling context for LLM generation with governance layer alongside](/images/2026/governed-rag-data-products/governed-rag-architecture.png)

The architecture has three layers:

**The retrieval layer** handles unstructured context: documents, policies, runbooks, product documentation. It uses a vector store indexed from data products (which are governed, SLA-backed datasets) rather than open-ended document crawls. Access policies at the retrieval layer filter returned chunks to those the requesting user is authorized to see.
...
Questions that require precise calculation (such as revenue, user counts, and conversion rates) should not route to the semantic layer for deterministic SQL generation.
...
Snowflake Horizon extends its governance framework to AI workloads through AI guardrails, which are policies that restrict what AI systems can access when using Cortex and AI-native features. For organizations using Snowflake as the data product backing for RAG, Horizon's AI guardrails add a policy layer limiting which subsets of authorized data can be included in AI context.

A sensitive financial table might be accessible to ANALYST role for SQL queries but excluded from AI context by guardrail policy, providing defense in depth that goes beyond standard role-based access.
...
This four-layer audit trail satisfies most enterprise audit requirements, which is far more complete than what an ungoverned RAG system can provide.
...
Governed RAG on data products is the enterprise-grade version of retrieval-augmented generation. It's not the easiest path to a working demo (since ungoverned RAG is faster to build), but it's the only version that produces responses an organization can trust, audit, and stand behind when someone asks how the AI came to a particular conclusion.
...
**Fixed-size chunking** divides documents into equal-length windows (e.g., 512 tokens) with optional overlap. It is simple but semantically arbitrary, as a chunk boundary might fall in the middle of a sentence or concept.
...
The routing logic is itself a classification model, which can be either a fine-tuned classifier or a smaller, fast LLM that categorizes the incoming question before routing:
