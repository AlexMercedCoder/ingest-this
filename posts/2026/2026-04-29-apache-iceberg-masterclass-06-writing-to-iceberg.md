...
date: "2026-04-29"
description: "<!-- Meta Description: Here is exactly how an engine writes to an Iceberg table, step by step, from data files through the atomic commit that makes ACID guarantees possible. -->"
author: "Alex Merced"
...
This is Part 6 of a 15-part [Apache Iceberg Masterclass](/posts/2026/2026-04-29-apache-iceberg-masterclass-01-table-formats). [Part 5](/posts/2026/2026-04-29-apache-iceberg-masterclass-05-hidden-partitioning) covered hidden partitioning. This article walks through the exact steps an engine takes when writing data to an Iceberg table, when the write becomes visible, and how concurrent writers are handled.
...
![The Iceberg write process from data file creation through the atomic catalog pointer swap](/images/2026/apache-iceberg-masterclass/06-writing-to-iceberg/write-process-flow.png)
...
For each new data file, the engine creates a manifest entry containing the file path, file size, row count, partition values (computed using the table's [partition transforms](/posts/2026/2026-04-29-apache-iceberg-masterclass-05-hidden-partitioning)), and column-level statistics (min, max, null count).
...
The engine asks the [catalog](/posts/2026/2026-04-29-apache-iceberg-masterclass-07-lakehouse-catalogs) to update its pointer from the old `metadata.json` to the new one. This is a compare-and-swap operation: the catalog checks that the current pointer matches what the engine expects, and only then updates it.
...
![How ACID works through the atomic metadata pointer swap](/images/2026/apache-iceberg-masterclass/06-writing-to-iceberg/atomic-commit-pointer-swap.png)

The pointer swap mechanism delivers all four ACID properties:

**Atomicity.** The entire write is visible or invisible. If the engine crashes after writing data files but before the pointer swap, the data files are orphans. They exist in storage but no metadata references them. Readers never see partial writes. A cleanup process (covered in [Part 10](/posts/2026/2026-04-29-apache-iceberg-masterclass-10-maintaining-iceberg)) can remove these orphans later.
...
![How two concurrent writers are resolved through optimistic concurrency with retry on conflict](/images/2026/apache-iceberg-masterclass/06-writing-to-iceberg/concurrent-write-conflict.png)
...
Eventually, old snapshots are expired (removed from the metadata) and their orphan files are cleaned up. This maintenance is covered in [Part 10](/posts/2026/2026-04-29-apache-iceberg-masterclass-10-maintaining-iceberg).
...
[Part 7](/posts/2026/2026-04-29-apache-iceberg-masterclass-07-lakehouse-catalogs) covers the catalog landscape in detail.
